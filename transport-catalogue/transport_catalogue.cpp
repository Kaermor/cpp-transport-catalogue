...
void TransportCatalogue::AddBus(const std::string& bus_id
    ...
    for (auto& stop : route_stops) {
        ...
        const_cast<Stop*>(stopname_to_stop_.at(stop))->buses.insert(buses_.back().name);
    }
...
const std::set<std::string_view> TransportCatalogue::GetStopInfo(const std::string_view stop_name) const{
